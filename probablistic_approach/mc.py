...
from support_functions.help_functions import validate_case_parameters, get_hdd_rad, get_effitiency_coefs,get_value_from_sheet, get_fixed_cost_by_key_from_sheet, get_coefficients, get_heating_types_helper, get_plant_types_helper1
from financial_analysis.Financial import get_electric_cost, get_pellet_cost, get_fuel_cost
...
def noise(constants, output, case, hdd_rad, vitamax, noise_type):
    ...
    for tmp in common_params:
        noise_server_common(output, case["common_params"], tmp, "common", dwelling_count, noise_type, id(case))
    ...
    for tmp in constant_params:
        noise_server_common(output, coefficients, tmp, "constants", dwelling_count, noise_type, id(case))
    ...
    for i in range(0, vitamax + 1):
        noise_server_hdd(output, i, hdd_rad["hdd"], id(case), noise_type)
        noise_server_rad(output, i, hdd_rad["rad"], id(case), noise_type)
        noise_server_tday(output, i, hdd_rad["tday"], id(case), noise_type)
        noise_server_dr(output, i, dr, id(case), noise_type)
...
def noise_server(output, case, value_name, changed, noise_type):
    ...
    base_current = case["current_params"][value_name]
    base_planned = case["planned_params"][value_name]

    std_dev_current = base_current * output["confidence"][value_name] / math.sqrt(ndwx)
    std_dev_planned = base_planned * output["confidence"][value_name] / math.sqrt(ndwx)

    noise_current = rumore(std_dev_current, noise_type)
    noise_planned = rumore(std_dev_planned, noise_type) if changed else noise_current

    output[id(case)]["current"][value_name] = base_current + noise_current
    output[id(case)]["planned"][value_name] = base_planned + noise_planned
...
def noise_server_common(output, values_dict, value_name, target, dwelling_count, noise_type, case_id):
    ...
    if output["changed"][value_name]:
        val = 1
    # todo call with values dict by id(case)
    tmp = \
...
def get_cb_changed_vc(project, value_name):
    ...
    cb_95_today = 0
    cb_95_final = 0
    changed = project[project_params_map[value_name] + "_noise_check"]
    if changed:
        cb_95_today = project[project_params_map[value_name] + "_confidence_today"] / 100
        cb_95_final = project[project_params_map[value_name] + "_confidence_final"] / 100
...
def get_changed_conf_value(project, name):
    ...
    return project[project_param_map[name] + "_noise_check"], \
           project[project_param_map[name] + "_confidence"]
...
def generate_noise(cases, constants, project, vitamax, year):
    noise_type = project["noise_flag"]
    index = 0
    noisematrix = {"confidence": {}, "changed": {}}
    if not project["uncertainty_advanced_flag"]:
    ...
    for case in cases:
        ...
        noisematrix[id(case)] = init_output(case, vitamax)
        index += 1
